...
- PDF parsing is local via `PyMuPDF`, so there is no OCR vendor charge per PDF.
- SQLite is local, so there is no managed database charge per PDF.
- Professional link fetching uses normal HTTP plus optional GitHub API access; there is no built-in paid enrichment vendor.
- The variable cost is almost entirely LLM token usage in `LLM_MODE=live`.

### Cost model

Using this code path:

- shared per-job LLM work: about `1,290` input tokens and `280` output tokens
- per-candidate live work: about `4,446` input tokens and `595` output tokens for an average resume in the bundled dataset
- larger resumes in the bundled dataset push candidate input closer to about `7,982` input tokens

These estimates are based on:

- the actual prompt files in `backend/app/prompts/`
- the orchestrator payload shapes
- the output schemas
- observed resume lengths in the bundled SQLite DB

### Estimated live cost per candidate PDF

As of `2026-06-05`, Chutes pricing pages list:

- `Qwen/Qwen2.5-Coder-32B-Instruct-TEE`: `$0.0245` / 1M input tokens and `$0.0978` / 1M output tokens
- `deepseek-ai/DeepSeek-V3.2-TEE`: `$0.28` / 1M input tokens and `$0.42` / 1M output tokens

Estimated cost per analyzed candidate PDF:

| Provider | Approx cost per PDF |
| --- | --- |
| `Qwen/Qwen2.5-Coder-32B-Instruct-TEE` | about `$0.00017` to `$0.00031` |
| `deepseek-ai/DeepSeek-V3.2-TEE` | about `$0.0015` to `$0.0030` |

Read that as:

- Qwen 32B TEE: about `0.017` to `0.031` cents per PDF
- DeepSeek V3.2 TEE: about `0.15` to `0.30` cents per PDF

These figures assume normal successful runs and do not include:

- extra repair calls when a provider returns invalid JSON
- extra retries from provider/network instability
- future provider price changes
- infrastructure costs outside Jobest itself

Triage-only runs are cheaper than full analysis because they use only the `triage_ranker` live call instead of the full deep-analysis pipeline.

## Scalability Notes

Scalability characteristics:

- Per-user parallelism is configurable in `/settings` with `parallel_agents_limit` from `1` to `10`.
- The analysis queue is in-memory and process-local (`backend/app/services/analysis_queue.py`).
- There is no global worker cap in code, only a per-user cap.
- Uploaded PDFs are stored on the local filesystem.
- Default persistence is SQLite in WAL mode.

In practice:

- For a single-machine deployment or MVP, this is a practical setup.
- For one active user, throughput scales roughly with `parallel_agents_limit` until provider latency or machine resources become the bottleneck.
- For multiple active users, one backend process can over-commit itself because the queue has no distributed broker and no hard global concurrency ceiling.
- SQLite and local uploads are not the right long-term choice for horizontally scaled multi-instance production.

## Submission Artifacts

- pitch deck: `docs/submission/Jobest_Pitch_Deck_C0nc3pt_Squad.pptx`
- video demo: `docs/submission/Jobest_demo.mp4`
- architecture diagram: `docs/submission/system-architecture.svg`
- agent framework diagram: `docs/submission/agent-framework-diagram.svg`
